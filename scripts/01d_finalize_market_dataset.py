...
def reset_date_index(df: pd.DataFrame) -> pd.DataFrame:
    """
    Reset a DatetimeIndex to an explicit 'Date' column robustly across
    pandas versions. Pandas 3.x may drop/preserve index names differently
    after .loc[] with a newly constructed DatetimeIndex.
    """
    out = df.copy()
    ...
    # Defensive fallback in case the index column is still named differently.
...
def main() -> int:
    ...
    for ticker in STOCKS:
        ...
        out_reset = reset_date_index(out)
        out_reset.to_csv(stock_path, index=False)

        panel_parts.append(out_reset)
    ...
    nifty_final_reset = reset_date_index(nifty_final)
    nifty_final_reset.to_csv(
    ...
    if "Date" not in panel.columns:
        raise RuntimeError(
            "Internal error: concatenated panel has no 'Date' column. "
            f"Columns found: {panel.columns.tolist()}"
        )
